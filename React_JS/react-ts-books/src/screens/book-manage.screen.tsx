...
interface BookManagerProps{
  showAddBookScreen : string | null
}


export const BookManageScreen = ({ showAddBookScreen}:BookManagerProps) => {

    const [selectedBook, setSelectedBook]=React.useState(null);

    const [books, updateBooks]= React.useState(booksList);

    const handleBookDelete=(id:string)=>{
      
        var remainingBooks= books.filter(a=>a.id!==id);
        updateBooks(remainingBooks);

    }

    const addNewBook = (newBook : any ) => {
    ...
    return (
        <div className='row g-0'>
            <div className='col col-5 author-list'>
                <BookList books={books} onBookSelect={(book:any)=>setSelectedBook(book)} />
            </div>
