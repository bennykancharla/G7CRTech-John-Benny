...
var {donnable}=require('../src/reusableTest');

should();

var { findPrimesInteractive,isPrimeSync } = require('../src/AsyncFuncs');


describe.only('findPrimesInteractive',function(){
    ...
    it('should return a unqiue id for each findPrimesInteractive calls',function(){
        ...
        var {id:id1}=findPrimesInteractive(2,10,response=>{});
        var {id:id2}=findPrimesInteractive(10,20,response=>{});
    ...
    it('should abort the process and return message:"abort" on received abort request from the client', done=>{
        ...
        let aborted=false;
        const {id,abort} = findPrimesInteractive(2,20000, response=>{
