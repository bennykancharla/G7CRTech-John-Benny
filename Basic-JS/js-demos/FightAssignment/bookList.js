...
function printMapResult(result){
    for(var i =0;i< result.length(); i++){
        var eachResult = result.get(i);
        console.log(eachResult)
    }
}


var mapTest = new LinkedList();
mapTest.append(1,2,3,5,7);

function square(num){
    return num**2
}
// printMapResult(mapTest.map(square))
// printMapResult(mapTest);

mapTest.forEach(square)
